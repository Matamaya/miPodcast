        ...
        <div className="w-full">
            ...
            <div className="w-full text-white font-sans max-w-7xl mx-auto">
                ...
                <div className="flex justify-between items-center mb-6">
                    ...
                    <button className="flex items-center gap-3 border border-zinc-600 rounded-full pl-5 pr-1 py-1 text-sm font-medium hover:bg-zinc-800 transition focus:outline-none focus:ring-2 focus:ring-emerald-500">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6">
                        ...
                        {playlistData.slice(1).map((track, idx) => {
                            ...
                            return (
                                <div
                                    ...
                                    role="button"
                                    tabIndex="0"
                                    aria-label={`Reproducir Episodio ${track.title}`}
                                    ...
                                    className="bg-[#1c1c1e] rounded-3xl p-6 lg:p-8 cursor-pointer hover:bg-[#252528] transition-colors group relative overflow-hidden flex flex-col focus:outline-none focus:ring-4 focus:ring-emerald-500"
                                >
                                    <div className="flex flex-wrap items-center gap-4 lg:gap-6 mb-4 text-xs font-semibold text-zinc-300">
                                        ...
                                        <span className="flex items-center gap-2">
                                            <svg aria-hidden="true" className="w-4 h-4 text-emerald-500" fill="currentColor" viewBox="0 0 24 24"><path d="M12 14c1.66 0 3-1.34 3-3V5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3z" /><path d="M17 11c0 2.76-2.24 5-5 5s-5-2.24-5-5H5c0 3.53 2.61 6.43 6 6.92V21h2v-3.08c3.39-.49 6-3.39 6-6.92h-2z" /></svg>
                                            {authors[idx]}
                                        </span>
                                        <span className="flex items-center gap-2">
                                            <span aria-hidden="true" className="w-2 h-2 rounded-full bg-red-500"></span>
                                    ...
                                    <p className="text-zinc-400 text-sm mb-8 line-clamp-2 md:pr-10">{descs[idx]}</p>
                    ...
                    <div
                        role="button"
                        tabIndex="0"
                        aria-label={`Reproducir Episodio Destacado ${playlistData[0].title}`}
                        ...
                        className="lg:col-span-5 rounded-3xl cursor-pointer group relative overflow-hidden h-full flex flex-col justify-end min-h-[500px] focus:outline-none focus:ring-4 focus:ring-emerald-500"
                    >
                        {/* Background Image */}
                        <div className="absolute inset-0">
                            <img
                                src="../portada.jpg"
                                alt="Portada del Episodio Destacado"
                            ...
                            <div className="flex items-center justify-between">
                                <span className="flex items-center gap-3 font-semibold text-sm bg-white text-black pl-2 pr-5 py-2 rounded-full hover:bg-zinc-200 transition-colors">
                                    <span className="w-8 h-8 flex items-center justify-center">
                                        <svg aria-hidden="true" className="w-5 h-5 ml-1" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
                                    </span>
                                    Escuchar Ahora
                                </span>
            ...
            {
                isOpen && (
                    <div ref={overlayRef} role="dialog" aria-modal="true" aria-label="Reproductor de Audio en pantalla completa" className="fixed inset-0 z-[100] flex flex-col md:flex-row bg-[#080808] text-white w-full h-full overflow-hidden shadow-2xl">
                        ...
                        <div className="relative w-full md:w-[40%] bg-gradient-to-br from-[#101962] via-[#351052] to-[#de141c] h-[40%] md:h-full flex flex-col justify-between p-6 md:p-12 z-20 overflow-hidden">
                            ...
                            <button
                                ...
                                className="flex items-center gap-3 text-white/70 hover:text-white transition group w-fit z-10 font-light tracking-widest text-sm md:text-xl">
                                <svg aria-hidden="true" className="w-6 h-6 md:w-8 md:h-8 group-hover:-translate-x-2 transition-transform opacity-70" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1">
                            ...
                            <div className="relative z-10 flex justify-between items-end w-full">
                                <button aria-label="Reproducir toda la playlist" className="flex items-center gap-2 text-[10px] md:text-xs font-bold tracking-[0.2em] hover:text-white/80 transition-colors uppercase">
                                    <svg aria-hidden="true" className="w-3 h-3 md:w-4 md:h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
                                    PLAY ALL
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="relative w-full md:w-[60%] bg-[#121212] h-[60%] md:h-full flex flex-col p-6 md:p-16 overflow-y-auto">
                            ...
                            {/* Cabecera */}
                            <div className="text-zinc-400 text-[10px] md:text-[11px] tracking-[0.2em] mb-4 md:mb-8 font-medium uppercase relative z-10">
                                PLAYLIST / <span className="text-[#de141c]">2026</span>
                            </div>

                            {/* Titulo Gigante */}
                            <div className="flex items-start gap-10 mb-8 overflow-hidden w-full relative">
                                <h2 aria-live="polite" className="text-white text-5xl md:text-6xl leading-none font-light tracking-tight flex items-start gap-4 z-10 whitespace-nowrap">
                                ...
                                {playlistData.map((track, i) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            aria-label={`Reproducir pista ${i + 1}: ${track.title}`}
                                            onClick={() => playTrack(i)}
                                            className={`flex items-center justify-between cursor-pointer px-4 py-3 transition-colors focus:outline-none focus:ring-2 focus:ring-[#de141c] rounded ${isActive
                                                ? 'text-white bg-zinc-900/40 rounded-sm -mx-4 backdrop-blur-sm border-l border-[#de141c]'
                                                : 'text-zinc-400 hover:text-white group'
                                                }`}
                                        >
                                            <div className="flex items-center gap-6 relative">
                                                {isActive ? (
                                                    <svg aria-hidden="true" className="w-3 h-3 md:w-4 md:h-4 text-[#de141c]" fill="currentColor" viewBox="0 0 24 24"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z" /></svg>
                                                ) : (
                                                    <svg aria-hidden="true" className="w-3 h-3 md:w-4 md:h-4 opacity-0 group-hover:opacity-100 transition-opacity" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
                                                )}
                            ...
                            <div className="absolute bottom-0 left-0 right-0 bg-[#121212]/95 backdrop-blur-3xl border-t border-zinc-800 p-6 md:p-10 flex flex-col gap-6 md:gap-8 z-30">
                                ...
                                <div className="flex justify-between items-center text-[9px] md:text-[11px] tracking-[0.15em] font-medium">
                                    ...
                                    <div className="text-zinc-400 tracking-widest" aria-live="polite">
                                        <span className="sr-only">Tiempo transcurrido:</span> {formatTime(currentTime)}
                                        <span className="text-zinc-700 mx-1" aria-hidden="true">/</span>
                                        <span className="sr-only">Duración total:</span> {formatTime(duration)}
                                    </div>
                                </div>
                                ...
                                <div className="flex justify-between items-end gap-6 md:gap-12">
                                    ...
                                    {/* Controles Playback */}
                                    <div className="flex items-center gap-4 md:gap-6 text-zinc-400 mb-1">
                                        {/* Prev */}
                                        <button onClick={handlePrev} aria-label="Pista anterior" className="hover:text-white transition group focus:outline-none focus:ring-2 focus:ring-white rounded">
                                            <svg aria-hidden="true" className="w-3 h-3 md:w-4 md:h-4 fill-current group-active:scale-90 transition-transform" viewBox="0 0 24 24">
                                                <path d="M6 6h2v12H6zm3.5 6l8.5 6V6z" /></svg></button>
                                        {/* Play/Pause */}
                                        <button onClick={togglePlay} aria-label={isPlaying ? 'Pausa' : 'Reproducir'} className="hover:text-[#de141c] transition text-white/90 group active:scale-90 focus:outline-none focus:ring-2 focus:ring-white rounded-full">
                                            {isPlaying ?
                                                <svg aria-hidden="true" className="w-5 h-5 md:w-6 md:h-6 fill-current text-[#de141c]" viewBox="0 0 24 24"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z" /></svg> :
                                                <svg aria-hidden="true" className="w-5 h-5 md:w-6 md:h-6 fill-current group-hover:text-[#de141c]" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
                                            }
                                        </button>
                                        {/* Next */}
                                        <button onClick={handleNext} aria-label="Pista siguiente" className="hover:text-white transition group focus:outline-none focus:ring-2 focus:ring-white rounded"><svg aria-hidden="true" className="w-3 h-3 md:w-4 md:h-4 fill-current group-active:scale-90 transition-transform" viewBox="0 0 24 24"><path d="M6 18l8.5-6L6 6v12zM16 6v12h2V6h-2z" /></svg></button>
                                    </div>
                                    ...
                                    >
                                        {waveData.map((h, i) => {
                                            ...
                                            return (
                                                <div key={i} style={{ height: `${h}px` }} aria-label="Barra de progreso" className={`flex-1 rounded-t-[1px] transition-colors duration-75 ${isActive ? 'bg-[#de141c]' : 'bg-zinc-700'}`}></div>
                                            )
                                        })}
                                    </div>
                                    ...
                                    <div className="flex items-center gap-2 hidden md:flex mb-1">
                                        <svg aria-hidden="true" className="w-4 h-4 md:w-5 md:h-5 fill-current text-zinc-400" viewBox="0 0 24 24"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z" /></svg>
                                        <label htmlFor="volume-range" className="sr-only">Control de volumen</label>
                                        <input
                                            id="volume-range"
                                            type="range"
                                            min="0"
